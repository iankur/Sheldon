...
isEnPassantCapture (OngoingGame color board _ lastMove) from to =
  Map.lookup (Move.to lastMove) board == Just (Figure Pawn (other color)) &&
  Move.to lastMove == Field (col to) (row to) &&
  Move.from lastMove == Field (col to) (row to + 2 * (row to - row from))
...
-- The most complex case 1handled by the mthod is the case of the Pawn moves.
